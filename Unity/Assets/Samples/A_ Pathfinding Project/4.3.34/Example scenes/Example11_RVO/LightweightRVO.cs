...
namespace Pathfinding.Examples
{
    ...
    public class LightweightRVO: MonoBehaviour
    {
        ...
        private NativeArray<float3> goals;

        /// <summary>Color for each agent</summary>
        private NativeArray<Color> agentColors;

        private NativeArray<float2> interpolatedVelocities;
        private NativeArray<float2> interpolatedRotations;
        ...
        public void CreateAgents(int num)
        {
            ...
            if (type == RVOExampleType.Circle)
            {
                ...
                {
                    Vector3 pos = new Vector3(Mathf.Cos(i * Mathf.PI * 2.0f / agentCount), 0, Mathf.Sin(i * Mathf.PI * 2.0f / agentCount)) *
                            math.lerp(innerCircleRadius, outerCircleRadius, UnityEngine.Random.value);
        ...
        [BurstCompile]
        private struct JobMoveAgents: IJob
        {
            ...
            [ReadOnly]
            public NativeArray<float3> agentTargetPoints;

            [ReadOnly]
            public NativeArray<float> agentMaxSpeeds;
            ...
            public void Execute()
            {
                for (int agentIndex = startIndex; agentIndex < endIndex; agentIndex++)
                {
                    ...
                    {
                        deltaPosition *= maxLength / length;
                    }
                    ...
                    if (vLength > agentMaxSpeeds[agentIndex] * 0.1f && vLength > 0)
                    {
                        ...
                        interpolatedRotations[agentIndex] = math.lerp(interpolatedRotations[agentIndex], interpolatedVelocities[agentIndex],
                            math.saturate(agentSpeeds[agentIndex] * deltaTime * 4f));
                    }
                }
            }
        }
        ...
        private struct JobGenerateMesh: IJob
        {
            [ReadOnly]
            public NativeArray<float2> interpolatedRotations;
            ...
            [WriteOnly]
            public NativeArray<Vertex> verts;
            ...
            public void Execute()
            {
                for (int i = startIndex; i < endIndex; i++)
                {
                    ...
                    if (math.all(forward == 0))
                    {
                        forward = new float3(0, 0, agentRadii[i]);
                    }
                    ...
                    verts[vc + 0] = new Vertex { position = orig + forward - right, uv = new float2(0, 1), color = color };

                    verts[vc + 1] = new Vertex { position = orig + forward + right, uv = new float2(1, 1), color = color };

                    verts[vc + 2] = new Vertex { position = orig - forward + right, uv = new float2(1, 0), color = color };

                    verts[vc + 3] = new Vertex { position = orig - forward - right, uv = new float2(0, 0), color = color };
        ...
        [BurstCompile]
        private struct JobSetAgentSettings: IJob
        {
        ...
        public void Update()
        {
            ...
            if (burstSim == null)
            {
                throw new System.Exception("The Lightweight RVO example requires the burst option to be enabled on the RVOSimulator");
            }
