        ...
        private class ProceduralTile
        {
            ...
            private Quaternion RandomYRot(ProceduralPrefab prefab)
            {
                return prefab.randomRotation == RotationRandomness.AllAxes
                        ? Quaternion.Euler(360 * (float) rnd.NextDouble(), 360 * (float) rnd.NextDouble(), 360 * (float) rnd.NextDouble())
                        ...
                        for (int sx = 0; sx < world.subTiles; sx++)
                        {
                            for (int sz = 0; sz < world.subTiles; sz++)
                            {
                                ...
                                float perl = Mathf.Pow(
                                    Mathf.PerlinNoise((px + pref.perlinOffset.x) * pref.perlinScale, (pz + pref.perlinOffset.y) * pref.perlinScale),
                                    pref.perlinPower);
