...
namespace Pathfinding.Examples.RTS
{
    ...
    {
        public Action<BTContext> action;

        public SimpleAction(Action<BTContext> action)
        {
            this.action = action;
        }
    ...
    {
        public Func<BTContext, bool> predicate;
    ...
    public struct Value<T>
    {
        ...
        {
            if (other.binding != null && binding == null)
            {
            ...
            {
                binding = other.binding = new Binding<T>();
            }
            else
            {
                throw new Exception("Too complex binding");
            }
        }

        public void Bind(Func<T> other)
        {
            if (binding != null)
            {
                throw new Exception("Already has a binding");
            }
            ...
            binding.setter = v =>
            {
                throw new InvalidOperationException("Trying to assign a value which has been bound as read-only (using a delegate)");
            };
        }
    ...
    public class FindClosestUnit: BTNode
    {
        ...
        private RTSUnit FindClosest(Vector3 point)
        {
            ...
            return new BTSelector(new BTNode[]
            {
                new HarvestMode()
                {
                    child = new BTSelector(new BTNode[]
                    {
                        ...
                        {
                            new Condition(ctx => ctx.unit.storedCrystals > 0), new BTSequence(new BTNode[]
                            {
                                dropoff, new BTMove(new Value<Vector3>(() => dropoff.target.value.transform.position)), new SimpleAction(ctx =>
                                {
                        ...
                        new BTSequence(new BTNode[]
                        {
                            ...
                            new BTSequence(new BTNode[]
                            {
                                reserve, new BTMove(new Value<Vector3>(() => reserve.target.value.transform.position)),
                                new Harvest
                                {
                                    resource = new Value<RTSHarvestableResource>(() => reserve.target.value.resource),
                                    duration = 5
                                }
                            })
                        })
                ...
                new BTSequence(new BTNode[] { dropoffQueue, new BTMove(new Value<Vector3>(() => dropoffQueue.target.value.transform.position)) })
            });
        }
    }
    ...
    public class HarvestMode: BTTransparent
    {
        protected override void OnTerminate(BTContext ctx)
        {
            ctx.unit.GetComponent<RVOController>().layer = RVOLayer.Layer2;
        ...
        protected override Status DoTick(BTContext ctx)
        {
            ...
            ctx.unit.GetComponent<RVOController>().layer = s == Status.Running? RVOLayer.Layer3 : RVOLayer.Layer2;
            return s;
        }
    }
    ...
    public class Harvest: BTNode
    {
        ...
        protected override Status DoTick(BTContext ctx)
        {
            ...
            {
                ctx.animator.SetBool("harvesting", false);
                if (ctx.animator.GetCurrentAnimatorStateInfo(0).IsName("RTSHarvesterHarvesting") ||
                    ctx.animator.GetCurrentAnimatorStateInfo(0).IsName("RTSHarvesterHarvestingExit"))
                {
                    return Status.Running;
                }
                else
