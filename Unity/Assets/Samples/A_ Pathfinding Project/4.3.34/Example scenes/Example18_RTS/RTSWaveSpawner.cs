        ...
        private IEnumerator Start()
        {
            ...
            for (int it = 0;; it++)
            {
                for (int i = 0; i < waves.Length; i++)
                {
                    ...
                    int from = i;
                    while (i + 1 < waves.Length && waves[i + 1].delay == 0)
                    {
                        i++;
                    }
                    ...
                    foreach (var wave in toSpawn)
                    {
                        for (int j = 0; j < wave.count; j++)
                        {
                            ...
                            rnd.y = 0;
                            var go = Instantiate(wave.prefab, wave.spawnPoint.position + rnd, wave.spawnPoint.rotation) as GameObject;
