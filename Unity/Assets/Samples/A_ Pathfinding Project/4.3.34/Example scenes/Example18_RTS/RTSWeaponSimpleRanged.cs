...
namespace Pathfinding.Examples.RTS
{
    ...
    public class RTSWeaponSimpleRanged: RTSWeapon
    {
        ...
        public override void Attack(RTSUnit target)
        {
            ...
            {
                Instantiate(sourceEffect, sourceEffectRoot != null? sourceEffectRoot.position : transform.position,
                    sourceEffectRoot != null? sourceEffectRoot.rotation
                            : Quaternion.LookRotation(target.transform.position - transform.position, Vector3.up));
            }

            if (targetEffect != null)
            {
                Instantiate(targetEffect, target.transform.position,
                    Quaternion.LookRotation(transform.position - target.transform.position, Vector3.up));
            }
