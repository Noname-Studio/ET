...
namespace Pathfinding.Examples.RTS
{
    public class RTSUnitManager
    {
        ...
        private bool batchSelection = false;

        private RTSUnit mActiveUnit;
        ...
        public RTSUnit activeUnit
        {
            ...
            set
            {
                if (value != mActiveUnit)
                {
                    if (mActiveUnit != null)
                    {
                        mActiveUnit.OnMakeActiveUnit(false);
                    }

                    mActiveUnit = value;
                    if (mActiveUnit != null)
                    {
                        mActiveUnit.OnMakeActiveUnit(true);
                    }
                }
            }
        }
        ...
        public void MoveGroupTo(List<RTSUnit> group, Vector3 destination, bool userOrder, MovementMode mode)
        {
            ...
            if (Vector3.Distance(destination, previousMean) > thresholdDistance)
            {
                ...
                PathUtilities.FormationPacked(positions, destination, group[0].radius * 1.1f);
            }
            else
            {
