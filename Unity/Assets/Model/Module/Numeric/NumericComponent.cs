...
namespace ET
{
    ...
    public class NumericComponent: Entity
    {
        ...
        public void Update(NumericType numericType)
        {
            ...
            int old = NumericDic[final];
            int result = (int) (((GetByKey(bas) + GetByKey(add)) * (100 + GetAsFloat(pct)) / 100f + GetByKey(finalAdd)) *
                (100 + GetAsFloat(finalPct)) / 100f * 10000);
            NumericDic[final] = result;
            Game.EventSystem.Publish(new EventType.NumbericChange() { Parent = Parent, NumericType = (NumericType) final, Old = old, New = result })
                    .Coroutine();
        }
    }
}