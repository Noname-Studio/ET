...
namespace ET
{
    ...
    public sealed class KService: AService
    {
        ...
        public uint TimeNow => (uint) (TimeHelper.ClientNow() - StartTime);
        ...
        {
            foreach (long channelId in idChannels.Keys.ToArray())
            {
                Remove(channelId);
            }
        ...
        {
            IPEndPoint ip = (IPEndPoint) ipEndPoint;
        ...
        private void Recv()
        {
            ...
            {
                int messageLength = socket.ReceiveFrom(cache, ref ipEndPoint);
 ...
 kChannel.RealAddress == null? kChannel.RemoteAddress : NetworkHelper.ToIPEndPoint(kChannel.RealAddress);
                        ...
                        case KcpProtocalType.ACK: // connect返回
                            ...
                            kChannel = GetByLocalConn(localConn);
                            if (kChannel != null)
                            {
                        ...
                        case KcpProtocalType.FIN: // 断开
                            ...
                            remoteConn = BitConverter.ToUInt32(cache, 1);
                            localConn = BitConverter.ToUInt32(cache, 5);
                            int error = BitConverter.ToInt32(cache, 9);
                        ...
                        case KcpProtocalType.MSG: // 断开
                            ...
                            {
                                // 通知对方断开
                                Disconnect(localConn, remoteConn, ErrorCode.ERR_KcpNotFoundChannel, (IPEndPoint) ipEndPoint, 1);
                                break;
                            }
        ...
        {
            KChannel channel;
            localConnChannels.TryGetValue(localConn, out channel);
            return channel;
        }

        protected override void Get(long id, IPEndPoint address)
        {
            if (idChannels.TryGetValue(id, out KChannel channel))
            {
                return;
            }
            ...
            try
            {
                ...
                channel = new KChannel(id, localConn, socket, address, this);
                idChannels.Add(id, channel);
                localConnChannels.Add(channel.LocalConn, channel);
            }
            catch (Exception e)
            {
        ...
        {
            if (!idChannels.TryGetValue(id, out KChannel kChannel))
            {
                return;
            }
            ...
            if (waitConnectChannels.TryGetValue(kChannel.RemoteConn, out KChannel waitChannel))
            {
                if (waitChannel.LocalConn == kChannel.LocalConn)
                {
                    waitConnectChannels.Remove(kChannel.RemoteConn);
                }
            }
        ...
        {
            KChannel channel = Get(channelId);
            if (channel == null)
            {
                return;
            }
        ...
        private void RemoveConnectTimeoutChannels()
        {
            ...
            foreach (long channelId in waitConnectChannels.Keys)
            {
                waitConnectChannels.TryGetValue(channelId, out KChannel kChannel);
                if (kChannel == null)
                {
                ...
                // 10秒连接超时
                if (TimeNow > kChannel.CreateTime + 10 * 1000)
                {
                    waitRemoveChannels.Add(channelId);
                }
            }
            ...
            {
                waitConnectChannels.Remove(channelId);
            }
        }
