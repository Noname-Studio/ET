...
{
    [LabelText("连击间隔时间(秒)")]
    public float Interval = 5; // 连击间隔时间（秒）
...
{
    [LabelText("顾客名称")]
    [AssetSelector(Paths = "Assets/Res/DB/Kitchen/Customer", ExcludeExistingValuesInList = true)]
    [HorizontalGroup("xx1")]
    [PropertyOrder(-5)]
    [HideLabelAttribute]
    [SerializeField]
    ...
    public CustomerProperty Customer => mCustomer;

    [LabelText("食物列表")]
    [ListDrawerSettings(ShowIndexLabels = false, NumberOfItemsPerPage = 1, Expanded = true)]
    [HorizontalGroup("xx1")]
    [HideLabelAttribute]
    [SerializeField]
...
public class Requirements
{
    ...
    [LabelText("是否允许使用垃圾桶")]
    public bool AllowUseTrash = true; // 是否允许使用垃圾桶

    [LabelText("收集金币")]
    [ShowIf("@Main.Type.HasFlag(LevelType.Coin)")]
    public int RequiredCoin;

    [LabelText("收集点赞")]
    [ShowIf("@Main.Type.HasFlag(LevelType.LikeCount)")]
    public int LikeCount; //

    [LabelText("限定时间")]
    [ShowIf("@Main.Type.HasFlag(LevelType.FixedTime)")]
    public int FixedTime; //

    [LabelText("流失顾客")]
    [ShowIf("@Main.Type.HasFlag(LevelType.LostCustomer)")]
    public int LostCustomer; //

    [LabelText("服务顾客个数")]
    [ShowIf("@Main.Type.HasFlag(LevelType.NumberOfCustomerService)")]
    public int NumberOfCustomerService;

    [LabelText("服务食物个数")]
    [ShowIf("@Main.Type.HasFlag(LevelType.NumberOfCompletedOrders)")]
...
{
    [BoxGroup("常用")]
    [BoxGroup("常用/Split/基础设定", true, false, 0)]
    [HorizontalGroup("常用/Split", 0.0f, 0, 0, 0, LabelWidth = 130)]
    ...
    [SerializeField]
    [LabelText("订单产生模式")]
    [ValueDropdown("EDITOR_OrderMode")]
    [VerticalGroup("常用/Split/基础设定/Left")]
    ...
    [SerializeField]
    [LabelText("难度")]
    [ValueDropdown("EDITOR_DifficultType")]
    [VerticalGroup("常用/Split/基础设定/Left")]
    ...
    [HideLabel]
    private Requirements mRequirements = new Requirements(); // 过关条件

    [LabelText("订单列表")]
    [SerializeField]
    [HideIf("OrderMode", OrderMode.Random)]
    [ListDrawerSettings(NumberOfItemsPerPage = 5)]
    private List<CustomerOrder> mOrders = new List<CustomerOrder>(); // 订单列表

    [SerializeField]
    [LabelText("奖励的物品列表")]
    private List<RewardData> mRewards = new List<RewardData>(); // 奖励的物品列表

    [SerializeField]
    [LabelText("神秘顾客列表")]
    private List<SecretCustomer> mSecretCustomers = new List<SecretCustomer>(); // 神秘顾客列表

    //[ShowInInspector, LabelText("顾客光顾最小间隔"),BoxGroup("顾客基础设定")] private float mMinCustomerInterval;
    [SerializeField]
    [HideLabel]
    [BoxGroup("顾客基础设定")]
    [FoldoutGroup("顾客基础设定/订单间隔范围")]
    private RangeFloat mOrderInterval = new RangeFloat(0, 0); // 订单的间隔范围

    [SerializeField]
    [HideLabel]
    [BoxGroup("顾客基础设定")]
    [FoldoutGroup("顾客基础设定/顾客扔垃圾的间隔")]
    private RangeInt mLitterInterval = new RangeInt(0, 0); // 顾客扔垃圾的间隔

    [SerializeField]
    [HideLabel]
    [BoxGroup("顾客基础设定")]
    [FoldoutGroup("顾客基础设定/订单出现时间衰减")]
    private DecayData mOrderDecay = new DecayData(); // 订单出现时间衰减,随时间加速顾客来的频率

    [SerializeField]
    [HideLabel]
    [BoxGroup("顾客基础设定")]
    [FoldoutGroup("顾客基础设定/耐心衰减参数")]
    private DecayData mWaitingDecay = new DecayData(); // 耐心衰减参数
    ...
    [SerializeField]
    [FoldoutGroup("厨具基础设定/制作时间衰减")]
    [BoxGroup("厨具基础设定")]
    [HideLabel]
    private DecayData mCookingDecay = new DecayData(); // 制作时间衰减

    [SerializeField]
    [FoldoutGroup("厨具基础设定/烧焦时间衰减")]
    [BoxGroup("厨具基础设定")]
    [HideLabel]
    private DecayData mBurnDecay = new DecayData(); // 烧焦时间衰减
    ...
    [SerializeField]
    [LabelText("延长关卡顾客列表")]
    private List<CustomerProperty> mExtendCustomers = new List<CustomerProperty>(); // 延长关卡顾客列表

    [FoldoutGroup("连击设定")]
    [HideLabel]
    [SerializeField]
    private ComboConfig mComboCfg = new ComboConfig();

    [ShowInInspector]
    [BoxGroup("备注")]
    [TextArea]
    [HideLabel]
    private string mComments = ""; // 关卡设计备注
...
public partial class LevelProperty
{
    ...
    {
        { "固定模式", OrderMode.OrderPool }, { "随机订单", OrderMode.Random }
    };

    private static ValueDropdownList<DifficultType> EDITOR_DifficultType = new ValueDropdownList<DifficultType>
    {
        { "简单难度", DifficultType.EasyLevel }, { "普通难度", DifficultType.MiddleLevel }, { "困难难度", DifficultType.HardLevel }
    };
