...
public class EnergyManager: Singleton<EnergyManager>
{
    ...
    /// 是否达到了自然上限体力
    /// </summary>
    public bool IsNormalMax => CurEnergy >= MaxEnergy;

    /// <summary>
    /// 是否达到了游戏最大上限体力
    /// </summary>
    public bool IsLimit => CurEnergy == MaxEnergy + ExtensionEnergy;

    /// <summary>
    /// 是否无限能量
    /// </summary>
    public bool IsInfine => Record.InfineEnergy > 0;

    /// <summary>
    /// 无限体力当前剩余秒数
    /// </summary>
    public TimeSpan InfineTimeSpan => TimeSpan.FromSeconds(Record.InfineEnergy) - TimeSpan.FromSeconds(TimeUtils.GetUtcTimeStamp());
    ...
    private void RefreshRecord()
    {
        ...
        InfineTimer.SetTarget(Record.InfineEnergy);
        DBManager.Inst.UpdateLocal(mRecord);
        SwitchTimer();
    }
