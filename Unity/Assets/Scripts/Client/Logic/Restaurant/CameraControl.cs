...
public class CameraControl : MonoBehaviour
{
	...
	{
		get { return mCacheTransform.position; }
		set
		{
			mCacheTransform.position = value;
	...
	public Vector3 DistancePointToRectangle(Vector2 point, Rect rect)
	{
		...
		//      ======+==========+======   --yMax
		//       VII  |    VI    |   V
		if (point.x < minX) {     // Region I, VIII, or VII
			if (point.y < minY) { // I
				//Debugger.Log("I");
				return new Vector3(minX,minY, FixedZ);
			}
			else if (point.y > maxY) { // VII
				//Debugger.Log("II");
		...
		else if (point.x > maxX) { // Region III, IV, or V
			if (point.y < minY) {  // III
				//Debugger.Log("III");
				return new Vector3(maxX,minY , FixedZ);
			}
			else if (point.y > maxY) { // V
				//Debugger.Log("V");
	...
	private void Smooth()
	{
		if (mMouseUp || mMouseDown || mMousePress || Input.touchCount > 0) return;
