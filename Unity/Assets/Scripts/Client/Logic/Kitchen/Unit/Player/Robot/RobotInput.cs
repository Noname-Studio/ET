...
namespace Kitchen.Robot
{
    public class RobotInput
    {
        ...
        public RobotInput(PlayerController playerController,Camera camera)
        {
            ...
            mCamera = camera;
            KitchenPlayerInputHandler = (InputHandler)mPlayerController.GetType().GetField("mInput",BindingFlags.Instance | BindingFlags.NonPublic).GetValue(mPlayerController);
            doJobDelegate = (Action<InputHandler,RaycastHit>)KitchenPlayerInputHandler.GetType().GetMethod("DoJob", BindingFlags.Instance | BindingFlags.NonPublic).CreateDelegate(typeof(Action<InputHandler,RaycastHit>));
        }
