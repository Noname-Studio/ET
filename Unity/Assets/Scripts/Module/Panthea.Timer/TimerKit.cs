...
public class TimerKit
{
    ...
    public UnityTimer UnityTimer { get; } = new UnityTimer();
    public TimerKit()
    {
        mMessage = MessageKit.Inst;
        mUpdateManger = UnityLifeCycleKit.Inst;
        mUpdateManger.AddUpdate(Update);
        //监听事件当游戏退出到后台得时候自动触发
    ...
    public void Invoke(string key, float interval,Action<InvokeTimer> action,bool invokeAtStart = true, int times = -1)
    {
        var timer = new InvokeTimer(key, interval, action,invokeAtStart);
        Start(timer, times);
    }
